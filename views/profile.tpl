...
<div class="col-md-3">
    % import random
    <img src="/images/avatars/{{str(user['user_id'])}}" class="img-thumbnail profile-avatar" alt="User avatar"
         width="300" ord="{{random.randint(1, 999)}}">
    % if loggedin and user['user_id']!=userinfo['user_id']:
    <br>
    <br>
    <a class="friendsbutton btn btn-default btn-block profile-avatar"
       id="{{'addfriend' if not myfriend else 'removefriend'}}-{{user['user_id']}}">
    ...
    <div class="btn-group">
        <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" style="margin-top:-7px;">
            <span class="glyphicon glyphicon-stats"></span> Всего сыграно: {{user['gameinfo']['beautiful']['total'][0]}}
            {{user['gameinfo']['beautiful']['total'][1]}}
        </button>
        <ul class="dropdown-menu" role="menu">
            % for sport_id in user['gameinfo']['sport_types']:
            <li>
                <a href="">{{user['gameinfo']['sport_types'][sport_id]}}: {{'
                    '.join(user['gameinfo']['beautiful'][sport_id])}}</a>
            </li>
            % end
        </ul>
    </div>
...
<div class="col-md-9">
    ...
    &nbsp;
    &nbsp;
    <small>Последний раз заходил{{'a' if user['sex']=='female' else ''}}: {{user['lasttime']}}</small>
    <br>
    % end
    ...
    <br>
    % if int(user['user_id'])==int(userinfo['user_id']) or userinfo['responsible'] or
    user['settings'].show_phone()=='all' or len(user['userlevel'].intersection({0,1}))>0:
    Телефон: {{user['phone']}}
    % end
    <br>
    <br>
                        ...
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered"
                                       style="font-size:90%; margin-bottom:0px;">
                                    <tr class="active">
                                        <td>№ игры</td>
                                        <td>Дата</td>
                                        <td>Название</td>
                                    ...
                                    <tr>
                                        <td><a href="/games?game_id={{game['game_id']}}" target="_blank">{{game['game_id']}}</a>
                                        </td>
                        ...
                        <div class="col-md-12">
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered"
                                    ...
                                    <tr
                                    {{'class=active' if not game['report']['reported'] else ''}}>
                                    <td><a href="/games?game_id={{game['game_id']}}"
                                           target="_blank">{{game['game_id']}}</a></td>
                                    <td>{{game['parsed_datetime'][0]}}</td>
                                    <td>{{game['description']}}</td>
                                    <td>{{game['sport_type']['title']}}</td>
                                    <td>{{game['court']['title']}}</td>
                                    <td>{{'<a href="/report?game_id={}">Отправлен</a>'.format(game['game_id']) if
                                        game['report']['reported'] else 'Ожидается'}}
                                    </td>
                                    </tr>
                                    % end
                                </table>
