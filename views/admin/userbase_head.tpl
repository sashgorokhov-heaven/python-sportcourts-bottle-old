<script>
  ...
  function initusertable (res, item) {
    ...
    if (item == false) {
      for (var i = 0; i < l; i++) {
        $('#userssearchtable').append('<tr><td><a target="_blank" href="/profile?user_id=' + res[i]["user_id"] + '">' + res[i]["user_id"] + '</a></td><td>' + res[i]["first_name"] + ' ' + res[i]["last_name"] + '</td><td>' + res[i]["phone"] + '</td><td><a href="mailto:' + res[i]["email"] + '">Написать</a></td><td><div class="btn-group"><button type="button" class="btn btn-default btn-xs dropdown-toggle" data-toggle="dropdown">Действие <span class="caret"></span></button><ul class="dropdown-menu" role="menu"><li><a href="#">Забанить</a></li><li><a href="#">Пригласить на игру</a></li><li><a href="#">Записать на игру</a></li></ul></div></td><td><span class="glyphicon glyphicon-remove"></span></td></tr>');
      }
    }
    else
    {
